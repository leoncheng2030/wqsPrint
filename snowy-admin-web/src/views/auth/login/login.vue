<template>
	<div class="login-wrapper">
					...
					<a-tabs v-model:activeKey="activeKey">
						<a-tab-pane key="userAccount" :tab="$t('login.accountPassword')">
							<a-form ref="loginForm" :model="ruleForm" :rules="rules">
								...
								<a-form-item v-if="captchaOpen === 'true'" name="validCode">
									<a-row :gutter="8">
										<a-col :span="17">
											<a-input
								...
								<a-form-item>
									<a class="xn-color-0d84ff" href="/findpwd">{{ $t('login.forgetPassword') }}？</a>
								</a-form-item>
								<a-form-item>
									<a-button :loading="loading" class="w-full" round size="large" type="primary" @click="login"
									>{{ $t('login.signIn') }}
									</a-button>
								</a-form-item>
							</a-form>
		...
		<div class="login-footer">
			<div class="version">
				<p>{{ sysBaseConfig.WQS_SYS_DEFAULT_DESCRRIPTION }}</p>
				<p>{{ sysBaseConfig.WQS_SYS_COPYRIGHT }} {{ sysBaseConfig.WQS_SYS_VERSION }}</p>
			</div>
		</div>
	</div>
</template>
...
const captchaOpen = ref(configData.SYS_BASE_CONFIG.WQS_SYS_DEFAULT_CAPTCHA_OPEN)
	...
	configApi
		...
		.then((data) => {
			if (data) {
				...
				captchaOpen.value = formData.value.WQS_SYS_DEFAULT_CAPTCHA_OPEN
				tool.data.set('WQS_SYS_BASE_CONFIG', formData.value)
...
// logo链接
const handleLink = (e) => {
	if (!sysBaseConfig.value.WQS_SYS_COPYRIGHT_URL) {
		e?.stopPropagation()
		e?.preventDefault()
	}
}
