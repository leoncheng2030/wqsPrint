...
export function usePrintOfficial() {
	 ...
	 * 支持大数据量优化和进度回调
	 */
	const generatePreviewHtml = async (template, data = {}, options = {}) => {
		...
		try {
			...
			// 使用打印管理器的优化版本生成预览
			const html = await printManager.generatePreview(targetTemplate, data, finalOptions)
	...
	const printMulti = async (printItems, options = {}) => {
		...
		try {
			...
			return printManager.printMulti(printItems, options);
		} catch (error) {
